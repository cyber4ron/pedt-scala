...
object Helper extends App {
  ...
  println(Base64.getEncoder.encodeToString(Source.fromFile("pedt/src/test/resources/task5.js").mkString.getBytes))
  ...
  println(MessageDigest.getInstance("MD5").digest(Source.fromFile("pedt/src/test/resources/task6.json").mkString.getBytes).map("%02X".format(_)).mkString.toLowerCase)
